...
// f3:
void first(list *xs){
  struct node *positionf;
  positionf = xs -> none -> next;
  xs -> current = positionf;
}
//f4:
void last(list *xs){
  struct node *positionl;
  positionl = xs -> none -> back;
  xs -> current = positionl;
...
void insertAfter(list *xs, item x){
  struct node *afternode;
    afternode = (struct node *) malloc(sizeof(struct node));
    afternode -> x = x;
    afternode -> next = xs-> current;
    afternode -> back = xs-> current -> next;
    if (xs -> none -> next == xs -> none){
      xs -> none -> next = afternode;
      xs -> none -> back = afternode;
    }
    else {
      xs -> current -> back -> next = afternode;
      xs -> current -> back = afternode;
    }
    xs -> current = afternode;
}

//f11:
void insertBefore(list *xs, item x){
    struct node *beforenode;
      beforenode = (struct node *) malloc(sizeof(struct node));
      beforenode -> x = x;
      beforenode -> back = xs-> current;
      beforenode -> next = xs-> current -> back;
      if (xs -> none -> back == xs -> none){
        xs -> none -> next = beforenode;
        xs -> none -> back = beforenode;
      }
      else {
        xs -> current -> next -> back = beforenode;
        xs -> current -> next = beforenode;
      }
      xs -> current = beforenode;
  }
}
...
bool deleteToAfter(list *xs){
  ...
  if (xs -> current = xs -> current -> next) {
    free(xs -> current);
    xs-> current -> next = xs -> current -> back;
    return true;
  }
}
bool deleteToBefore(list *xs){
  ...
  if (xs -> current = xs -> current -> back) {
    free(xs -> current);
    xs-> current -> back = xs -> current -> next;
    return true;
}
